...
export class MoralisService {
  ...
  static async getWalletTokens(
    ...
    const response = await axios.get<TokenResponse>(
      `${this.BASE_URL}/wallets/${walletAddress}/tokens`,
      {
        params: { chain },
        headers: {
  ...
  static async getWalletHistory(
    ...
    const response = await fetch(
      `https://deep-index.moralis.io/api/v2.2/wallets/${address}/history?chain=${chainId}&limit=5`,
      {
        headers: {
    ...
    data.result = data.result.map((transaction: Transaction) => ({
      ...transaction,
      erc20_transfers: transaction.erc20_transfers.filter(
        transfer => transfer.verified_contract && !transfer.possible_spam
      ),
    }))
