...
import { BASE_TOKENS, BASE_TOKENS_BY_SYMBOL, MAX_ALLOWANCE, AFFILIATE_FEE, FEE_RECIPIENT } from "../../src/constants";
...
export default function PriceView({
  ...
  }, [sellTokenObject.address, buyTokenObject.address, parsedSellAmount, parsedBuyAmount, chainId, sellAmount, setPrice, FEE_RECIPIENT, AFFILIATE_FEE]);
      ...
      <div className="container mx-auto p-10">
        ...
        <div className="bg-slate-200 dark:bg-slate-800 p-4 rounded-md mb-3">
          ...
          <section className="mt-4 flex items-start justify-center">
            ...
            <Image alt={sellToken} className="h-9 w-9 mr-2 rounded-md" src={BASE_TOKENS_BY_SYMBOL[sellToken].logoURI} width={9} height={9} />

            <div className="h-14 sm:w-full sm:mr-2">
              <select value={sellToken} name="sell-token-select" id="sell-token-select" className="mr-2 w-50 sm:w-full h-9 rounded-md" onChange={handleSellTokenChange}>
          ...
          <section className="flex mb-6 mt-4 items-start justify-center">
            ...
            <Image alt={buyToken} className="h-9 w-9 mr-2 rounded-md" src={BASE_TOKENS_BY_SYMBOL[buyToken].logoURI} width={9} height={9} />
            <select name="buy-token-select" id="buy-token-select" value={buyToken} className="mr-2 w-50 sm:w-full h-9 rounded-md" onChange={(e) => handleBuyTokenChange(e)}>
              {/* <option value="">--Choose a token--</option> */}
              {BASE_TOKENS.map((token) => {
                return (
                  <option key={token.address} value={token.symbol.toLowerCase()}>
          ...
          <div className="text-slate-400">
            {buyTokenTax.buyTaxBps !== "0" && <p>{BASE_TOKENS_BY_SYMBOL[buyToken].symbol + ` Buy Tax: ${formatTax(buyTokenTax.buyTaxBps)}%`}</p>}
            {sellTokenTax.sellTaxBps !== "0" && <p>{BASE_TOKENS_BY_SYMBOL[sellToken].symbol + ` Sell Tax: ${formatTax(sellTokenTax.sellTaxBps)}%`}</p>}
          </div>
        </div>

        {taker ? (
          <ApproveOrReviewButton
            sellTokenAddress={BASE_TOKENS_BY_SYMBOL[sellToken].address}
            taker={taker}
            onClick={() => {
              setFinalize(true);
            }}
              ...
              return (
                ...
                >
                  {(() => {
                    if (!connected) {
                      return (
                        <button className="w-full bg-blue-600 text-white font-semibold p-2 rounded hover:bg-blue-700" onClick={openConnectModal} type="button">
                          Connect Wallet
                        </button>
                      );
                    }
                      ...
                      <div style={{ display: "flex", gap: 12 }}>
                        <button onClick={openChainModal} style={{ display: "flex", alignItems: "center" }} type="button">
                          {chain.hasIcon && (
                            ...
                            >
                              {chain.iconUrl && <Image src={chain.iconUrl} alt={chain.name ?? "Chain icon"} width={12} height={12} layout="fixed" />}
                            </div>
                          )}
                          {chain.name}
                        </button>
  ...
  function ApproveOrReviewButton({ taker, onClick, sellTokenAddress, disabled, price }: { taker: Address; onClick: () => void; sellTokenAddress: Address; disabled?: boolean; price: any }) {
    ...
    const { data: writeContractResult, writeContractAsync: writeContract, error } = useWriteContract();
